...
"${JAVA_HOME:+$JAVA_HOME/bin/}javac" --release 11 -cp "$MIUIX_CP" -d "$MIUIX_DIR/build/classes" "$MIUIX_DIR"/tools/*.java
if [[ "${1:-}" == "gpu" ]]; then
    export __GL_THREADED_OPTIMIZATIONS="${__GL_THREADED_OPTIMIZATIONS:-0}"
    exec "${JAVA_HOME:+$JAVA_HOME/bin/}java" -cp "$MIUIX_DIR/build/classes:$MIUIX_CP" io.github.timer_err.qml4j.demo.MiuixGpuChecks "$MIUIX_DIR"
fi
